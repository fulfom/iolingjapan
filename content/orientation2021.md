...
###### 面接
選抜参加者の~~上位約30名~~銅賞相当以上の参加者のみ，本人確認・不正の防止のため終了後Zoomで数分程度の個人面接を行います．
...
競技の流れは[JOL2021事前準備](/demo2021/)で説明しています．
<!---
ここでは2020年のJOLの流れを説明します。
...
|---|---|
|要準備:| ネット環境、閲覧デバイス |   
|推奨環境:| 画面の大きな閲覧デバイス（スマートフォンは非推奨）<br> プリンターと筆記用具 or スタイラス・PDF編集ツール（Drawboardなど）|
{{</table>}}

#### 1. デモ試験

正常に試験を受けられることを確認するため事前にデモを行います。[受験者情報](/account)から開始してください。


#### 2. 本番
JOLは12/28（土）にオンラインで実施されます。問題の配布などは[受験者情報](/account)で行います。    


##### 2.1 筆記試験
試験が始まる前の12:30に集合し、正しく動作することを確認してください。   

試験は13:00から15:00までの2時間です。13:00になったら受験者情報に問題のpdfと解答用紙へのリンクが表示されます。解答用紙はGoogleスプレッドシートを使用します。解答は自動で保存され、解答終了時刻になった時点でのシートの状態が解答として記録されます。

##### 2.2 面接
筆記試験の結果に基づき、およそ銅賞ボーダー以上の{{< icon flag 選抜参加 >}}のかたを対象にZoomで面接を実施します。面接対象者に選ばれた場合、採点が終了し次第受験者情報のページで面接の対象に選ばれたこと、面接開始予定時間、面接Zoomへのリンクをお伝えします。面接開始時間より前にZoomに入り、メインルームで待機していてください。時間になったら面接用のブレイクアウトルームへ招待されます。
面接は5分ほど行われます。面接の内容は録画されます。内容によって加点・減点されることはありませんが、不正が発覚した場合は失格となります。

##### 2.3 結果通知
試験の結果は12/28日中に受験者情報のページで発表されます。結果が掲載されたらメールで通知します。
--->
