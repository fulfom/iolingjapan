...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    firebase.auth().onAuthStateChanged(async (user) => {
        if(user){
            ...
            await firebase.database().ref("/orders/jol2022/" + user.email.replaceAll('.','=')).on("value", async (snapshot2) => {
...
async function transitionToMainmenu(){
    ...
    await firebase.database().ref("/users/" + user.uid).update({entry: "mainmenu2022"});
