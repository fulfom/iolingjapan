...
const Links = ({ user, uid }: { user: any, uid: string }) => {
    const [publishDemo, publishDemoloading, publishDemoerror] = useObjectVal<PublishDemoType>(ref(db, '/contests/jol2025/publish/demo/'));
    const [demoUser, demoUserloading, demoUsererror] = useObjectVal<demoUserType>(ref(db, `/contests/jol2025/demo/${uid}/`));
            ...
            onValue(ref(db, '/orders/jol2025/' + user.email!.replace(/\./g, '=').toLowerCase()), (paid) => {
                if (paid.val()) {
                    onValue(ref(db, '/contests/jol2025/users/' + user.uid), (snapshot1) => {
                        ...
                        update(ref(db, '/contests/jol2025/demolog/' + user.uid), {
