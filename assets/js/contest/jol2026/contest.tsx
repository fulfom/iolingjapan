...
rootQA.render(
    <QA contId="jol2026" site="contest" />
);
...
const Links = ({ user, uid }: { user: any, uid: string }) => {
    const [publish, publishloading, publisherror] = useObjectVal<PublishType>(ref(db, '/contests/jol2026/publish/contest/'));
    const [answerSheet, answerSheetloading, answerSheeterror] = useObjectVal<string>(ref(db, `/contests/jol2026/contest/${uid}/answerSheet`));
    const [alternateAnswerSheet, setAlternateAnswerSheet] = useState<boolean>(false);

    const rug: number = publish && publish.rug || 0;

    const expiryTimestamp = useMemo(() => {
        const end = new Date("2025-12-29T15:00:00+09:00");
    ...
    useEffect(() => {
        if (problem) {
            ...
            push(ref(db, `/contests/jol2026/log/contest/${uid}/problem`), serverTimestamp())
        }
    }, [problem])
    useEffect(() => {
        if (answerSheet) {
            window.open(answerSheet, '_blank');
            push(ref(db, `/contests/jol2026/log/contest/${uid}/answerSheet`), serverTimestamp())
        }
    }, [answerSheet])
...
document.addEventListener("DOMContentLoaded", (event) => {
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            onValue(ref(db, '/orders/jol2026/' + user.email!.replace(/\./g, '=').toLowerCase()), (paid) => {
                if (paid.val()) {
                    onValue(ref(db, '/contests/jol2026/users/' + user.uid), (snapshot1) => {
                        ...
                        // update(ref(db, '/contests/jol2026/log/contest/' + user.uid), {
                        //     timestamp: serverTimestamp()
                        // })
                        push(ref(db, `/contests/jol2026/log/contest/${user.uid}/siteVisit`), serverTimestamp())
                    }, {
                        onlyOnce: true
                    });
                }
                else {
                    alert("JOL2026応募アカウントではありません．アカウントを間違えている場合はログインし直してください．")
