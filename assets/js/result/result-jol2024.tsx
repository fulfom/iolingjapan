...
const scores = {
    avgs: {
        flag: {
            whole: [51.88, 16.82, 15.01, 8.55, 5.34, 6.24],
            gold: [82.03, 19.69, 19.25, 15.71, 13.87, 13.50],
            silver: [69.86, 19.26, 17.32, 13.57, 8.29, 11.42],
            bronze: [62.10, 18.95, 17.33, 10.83, 6.97, 8.02],
            honourable: [55.20, 18.31, 16.11, 9.12, 5.28, 6.38],
        },
        award: {
            whole: [51.57, 17.13, 14.11, 7.88, 5.57, 6.88],
            gold: [88.09, 19.88, 19.94, 16.25, 15.96, 16.06],
            silver: [73.09, 19.61, 18.19, 13.28, 10.04, 11.97],
            bronze: [64.12, 18.96, 16.74, 10.54, 8.31, 9.57],
            honourable: [56.75, 18.60, 15.00, 9.58, 4.95, 8.60],
        },
        all: {
            whole: [51.83, 16.87, 14.87, 8.44, 5.37, 6.34]
        }
    },
    ...
    borders: {
        ...
        flag: [52, 58.5, 65.95, 76, 96.5],
        award: [52.2, 61.1, 69.3, 82.5, 100],
    }
}
...
function UserForm({ user, udb }: { user: User, udb: any }) {
    ...
    const onSubmit = async (data: any) => {
        await Promise.all([
            update(ref(db, "/contests/jol2024/users/" + user.uid), {
                ...data
            }),
    ...
    return (
        <div>
            ...
            <p>賞を獲得された方には賞状をお送りします．以下の氏名{!udb.isCertificateNecessary && "・郵送先住所"}をご確認いただき，修正が必要な場合は 2024年2月11日（日）までに修正してください．修正は以下の情報を書き換えて修正ボタンを押すことで完了します．</p>
...
function ResultChart({ userResult }: { userResult: Result }) {
    ...
    const data: ChartData<"radar"> = {
        ...
        datasets: [
            {
                label: 'あなた',
                data: toRate(userResult.sums).slice(1),
                fill: false,
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged(async v => {
            if (v) {
                setUser(v);
                //fetch data from db
                const snapshot = await get(ref(db, "/contests/jol2024/users/" + v.uid));
...
type Result = {
    participation: "onhold" | "" | "absent";
    spot: "flag" | "award";
    award: keyof typeof AWARD_NAME | "";
    aplo: "A" | "B" | "";
    sums: number[];
    answers?: string[];
    message?: string;
...
function App({ user, udb }: { user: User, udb: any }) {
    const [userResult, userResultLoading, userResultError] = useObjectVal<Result>(ref(db, "/contests/jol2024/results/" + user.uid), {
        transform: (val) => {
            return ({
                ...val,
                sums: val.sums.map((v: number) => Number(v)),
            })
        }
    });

    //award name
    const awardName = !userResultLoading && userResult ?
        userResult.award !== "" ?
            `${userResult.spot === "flag" ? "選抜" : "オープン"}枠${AWARD_NAME[userResult.award]}賞` : "受賞なし" :
        "";
    ...
    const videos: Video[] = useMemo(() => {
        ...
        if (userResult.grand) {
            tmp.push({
                src: `/video/jol2024-result/${userResult.spot}-best${userResult.grand}.mov`,
                alt: `${userResult.spot === "flag" ? "選抜" : "オープン"}枠最優秀賞（${userResult.grand}位）`
            });
        }
        // 優秀賞
        if (userResult.award) {
            tmp.push({
                src: `/video/jol2024-result/${userResult.spot}-${userResult.award}${(userResult.spot === "flag" && userResult.award === "silver") ? userResult.aplo ? "-aplo" : "-notaplo" : ""}.mov`,
                alt: awardName
            });
        }
        // ジュニア奨励賞
        if (userResult.junior) {
            tmp.push({
                src: `/video/jol2024-result/junior.mov`,
                alt: "ジュニア奨励賞"
            });
        }
        // 地区賞
        if (userResult.awardByArea) {
            tmp.push({
                src: `/video/jol2024-result/${userResult.awardByArea}.mov`,
                alt: `${userResult.awardByArea}地区賞`
            });
        }
        if (hasHmAward) {
            hmAward.forEach((v, i) => {
                if (v) {
                    tmp.push({
                        src: `/video/jol2024-result/${userResult.spot}-${i + 1}k.mov`,
                        alt: `第${i + 1}問敢闘賞`
                    });
                }
            });
        ...
        if (tmp.length === 0) {
            return [{
                src: `/video/jol2024-result/${userResult.spot}-none.mov`,
                alt: awardName
            }];
        } else {
            return tmp;
    ...
    return (<>
        ...
        {!userResultLoading ? userResult && <>
            {userResult.participation === "" && <>
                <VideoArea videos={videos}></VideoArea>
                {userResult.spot === "flag" && (
                    userResult.aplo ?
                        <p>2024年4月7日（日）実施のアジア太平洋言語学オリンピック2024（二次選抜）にご招待します．ただし，APLOは別途参加資格の確認があり，参加資格を満たす場合にのみ公式に参加ができます．詳しいことが決まり次第，メールでご連絡差し上げます．</p>
            ...
            {userResult.participation === "absent" && <p>あなたはJOL当日に参加していません．</p>}
        </> : <p>読み込み中...</p>}
    </>)
}
