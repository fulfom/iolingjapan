...
const scores = {
    avgs: {
        flag: {
            whole: [46.84, 14.92, 15.41, 7.07, 4.62, 4.83],
            gold: [82.60, 18.99, 17.32, 17.50, 16.45, 12.34],
            silver: [66.82, 17.84, 17.21, 12.97, 10.76, 8.04],
            bronze: [56.24, 17.27, 16.90, 10.33, 5.74, 6.01],
            honourable: [49.15, 16.11, 15.99, 8.46, 3.97, 4.62],
        },
        award: {
            whole: [45.77, 14.04, 14.69, 6.99, 5.00, 5.05],
            gold: [90.68, 19.77, 18.73, 18.64, 19.09, 14.45],
            silver: [73.67, 18.75, 16.13, 15.42, 14.00, 9.38],
            bronze: [55.49, 15.74, 16.18, 9.78, 7.09, 6.71],
            honourable: [47.50, 14.86, 15.39, 7.92, 4.22, 5.11],
        },
        all: {
            whole: [46.63, 14.75, 15.27, 7.05, 4.69, 4.87],
        }
    },
    ...
    borders: {
        ...
        flag: [47, 52, 62, 75.5, 98],
        award: [46, 50, 65.5, 84, 100],
    }
}
...
const color = {
    bg: {
        ...
        honourable: "rgba(56, 187, 142, 0.2)"
    },
    border: {
...
function UserForm({ user, udb }: { user: User, udb: any }) {
    ...
    return (
        <div>
            ...
            <p>賞を獲得された方には賞状をお送りします．以下の氏名{!udb.isCertificateNecessary && "・郵送先住所"}をご確認いただき，修正が必要な場合は 2026年2月1日（日）までに修正してください．修正は以下の情報を書き換えて修正ボタンを押すことで完了します．</p>
...
function ResultChart({ userResult }: { userResult: Result }) {
    ...
    const scoreGradationTotal = (borders: number[]) => {
        return [
            "to right",
            `#eee ${borders[0]}%`,
            `${color.bg.honourable} ${borders[0]}% ${borders[1]}%`,
...
function App({ user, udb }: { user: User, udb: any }) {
    const [userResult, userResultLoading, userResultError] = useObjectVal<Result>(ref(db, "/history/" + user.uid + "/jol2026/"));
    ...
    const videos: Video[] = useMemo(() => {
        ...
        if (!userResult.award) {
            return [{
                src: `/video/jol2026-result/${userResult.spot}-none.mp4`,
                alt: "受賞なし"
            }];
        }
        let tmp: Video[] = [];
        ...
        if (userResult.award.includes("gold")) {
            tmp.push({
                src: `/video/jol2026-result/${userResult.spot}-gold.mp4`,
                alt: "金賞"
            });
        }
        if (userResult.award.includes("silver")) {
            tmp.push({
                src: `/video/jol2026-result/${userResult.spot}-silver.mp4`,
                alt: "銀賞"
            });
        }
        if (userResult.award.includes("bronze")) {
            tmp.push({
                src: `/video/jol2026-result/${userResult.spot}-bronze.mp4`,
                alt: "銅賞"
            });
        }
        if (userResult.award.includes("honourable")) {
            tmp.push({
                src: `/video/jol2026-result/${userResult.spot}-honourable.mp4`,
                alt: "努力賞"
            });
        }
        ...
        if (userResult.award.includes("junior")) {
            tmp.push({
                src: `/video/jol2026-result/junior.mp4`,
                alt: "ジュニア奨励賞"
            });
        }
        // 地区賞
        if (userResult.award.includes("area")) {
            tmp.push({
                src: `/video/jol2026-result/${userResult.area}.mp4`,
        ...
        if (userResult.award.includes("hm")) {
            tmp.push({
                src: `/video/jol2026-result/${userResult.spot}-k.mp4`,
                alt: `敢闘賞`
            });
        }
                    ...
                    <VideoArea videos={videos}>
                        {userResult.spot === "flag" && (
                            userResult.award && userResult.award.includes("aplo") ?
                                <p>2026年3月29日（日）実施のアジア太平洋言語学オリンピック2026（二次選抜）にご招待します．ただし，APLOは別途参加資格の確認があり，参加資格を満たす場合にのみ公式に参加ができます．詳しいことが決まり次第，メールでご連絡差し上げます．</p>
                                : <p>（アジア太平洋言語学オリンピック2026への招待はありません）</p>
                        )}
                    </VideoArea>
